...
extern "C" void action_fechar_form_cluster(lv_event_t *e) {
  ...
  Serial.println("Fechando form de cluster...");
}
...
extern "C" void action_salvar_form_cluster(lv_event_t *e) {
  ...
  lv_event_send(objects.keyboard_1, LV_EVENT_CANCEL, NULL);
  Cluster item = {String(lv_textarea_get_text(objects.ed_cluster_nome)), String(lv_textarea_get_text(objects.ed_cluster_http)), "abcd1234", String(lv_textarea_get_text(objects.ed_cluster_port)).toInt()};
