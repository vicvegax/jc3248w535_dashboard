...
void lista_clusters() {
  ...
  for(int i = 0; i < total; i++) {
    ...
    btn = lv_list_add_btn(objects.ls_clusters, NULL, item.nome.c_str());
...
static void bt_cluster_cb(lv_event_t *e) {
  ...
  if(code == LV_EVENT_CLICKED) {
    ...
    uint32_t total_itens = lv_obj_get_child_cnt(objects.ls_clusters);
    for(uint32_t i = 0; i < total_itens; i++) {
        lv_obj_t * item = lv_obj_get_child(objects.ls_clusters, i);
